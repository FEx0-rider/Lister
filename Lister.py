...
def addBigWords(importedWords): # creates big versions of given words
    importedWordsList = []

    for i in importedWords:

        def capitalizeText(text, position): # capitalize letter x in text
    
            if position > 0 and position <= len(text):
                text = text[:position-1] + text[position-1].upper() + text[position:]
            return text
        
        for x in range(len(str(i))):
            importedWordsList.append(capitalizeText(i, x + 1))

        importedWordsList.append(i)
        importedWordsList.append(i.upper()) # upper case

        importedWordsList.append(i[::-1]) # inverted word
        importedWordsList.append(i[::-1].upper()) # inverted word upper case

        v1 = ''.join([p.upper() if i % 2 == 0 else p.lower() for i, p in enumerate(i)]) # ExPeRiMeNt
        v2 = ''.join([p.lower() if i % 2 == 0 else p.upper() for i, p in enumerate(i)]) # eXpErImEnT

        importedWordsList.append(v1)
        importedWordsList.append(v2)

    importedWordsList = list(set(importedWordsList)) # deleting repeating charakters

    return importedWordsList


def glueing(lettersListLowerUpper, symbols, importedWords):
...
if __name__ == "__main__":
    try:
        ...
        lettersListLowerUpper = decomposition(importedWords) # from given words makes charakters
        importedWords = addBigWords(importedWords) # from given words makes special edited words

        
        """
        symbols = @#$%^...
        importedWords = Example...
        lettersListLowerUpper = EX...
        
        fileName = Example.txt
        
        """
        print(importedWords, symbols, lettersListLowerUpper)
        glueing(lettersListLowerUpper, symbols, importedWords)
